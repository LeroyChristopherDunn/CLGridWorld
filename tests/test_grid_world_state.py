...
class TestGridWorldState(TestCase):
    ...
    def test_given_valid_parameters_should_correctly_create_state(self):
        ...
        self.assertEqual(shape, state[GridWorldState.SHAPE_KEY], "shape not equal")
        self.assertEqual(player_coords, state[GridWorldState.PLAYER_COORDS_KEY], "player coords not equal")
        self.assertEqual(key_coords, state[GridWorldState.KEY_COORDS_KEY], "key coords not equal")
        self.assertEqual(lock_coords, state[GridWorldState.LOCK_COORDS_KEY], "lock coords not equal")
        self.assertEqual(pit_coords, state[GridWorldState.PIT_COORDS_KEY], "pit coords not equal")
        self.assertEqual(nw_beacon, state[GridWorldState.NW_BEACON_COORDS_KEY], "nw beacon coords not equal")
        self.assertEqual(ne_beacon, state[GridWorldState.NE_BEACON_COORDS_KEY], "ne beacon coords not equal")
        self.assertEqual(sw_beacon, state[GridWorldState.SW_BEACON_COORDS_KEY], "sw beacon coords not equal")
        self.assertEqual(se_beacon, state[GridWorldState.SE_BEACON_COORDS_KEY], "se beacon coords not equal")
        self.assertEqual(0, state[GridWorldState.HAS_KEY_KEY], "has key not equal")
